...
- All the `MultiMeshInstance3D` features such as a unique transform (scale, rotation, and posistion) per instance.
...
- `MultiMeshInstance3D's custom_data is used by this plugin so you will not have access to it.

## MutiMeshInstance3D custom_data

`MultiMeshInstance3D's custom_data is used by this plugin.  Here is how it is used:

custom_data.r = animation offset
custom_data.g = animation start frame
custom_data.b = animation end frame
custom_data.a = alpha of mesh
