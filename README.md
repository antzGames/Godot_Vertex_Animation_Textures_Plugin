...
> [!IMPORTANT]  
> I have 2 VAT Godot plugins.  This one and the OpenVAT for Godot Plugin which relies on the OpenVAT Blender Toolkit that no longer appears to be maintained.  Recommend to use this plugin if undecided.  Also, VATs that are generated by OpenVAT are not compatible with this plugin, so you will need to rebake your animation using the plugins that this plugin requires.  The API is similar though.
